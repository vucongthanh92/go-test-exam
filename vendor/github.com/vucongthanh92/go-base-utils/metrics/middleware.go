...
var (
	...
	metricRequestTotalSeconds = "http_requests_total_seconds"
	metricURIRequestTotal     = "http_uri_request_total"
	metricRequestBody         = "http_request_body_total"
	metricResponseBody        = "http_response_body_total"
	metricRequestDuration     = "http_request_duration_seconds"
	metricSlowRequest         = "http_slow_request_total"
	metricErrorRequest        = "http_error_request_total"
	metricSuccessRequest      = "http_success_request_total"
)
...
func (m *Monitor) initGinMetrics() {
	...
	monitor.AddMetric(&Metric{
		Type:        Counter,
		Name:        metricRequestTotalSeconds,
		Description: "all the server received request num with every second.",
		Labels:      []string{"uri", "method", "code"},
	})

	monitor.AddMetric(&Metric{
		Type:        Counter,
		Name:        metricErrorRequest,
		Description: "all the server received error request num.",
		Labels:      []string{"uri", "method"},
	})

	monitor.AddMetric(&Metric{
		Type:        Counter,
		Name:        metricSuccessRequest,
		Description: "all the server received success request num.",
		Labels:      []string{"uri", "method"},
	})
...
func (m *Monitor) ginMetricHandle(ctx *gin.Context, start time.Time) {
	...
	// ignore metrics call
	if ctx.FullPath() == m.metricPath {
		return
	}
	// ignore health check
	if strings.Contains(ctx.FullPath(), "/live") ||
		strings.Contains(ctx.FullPath(), "/ready") {
		return
	}

	// set request total
	_ = m.GetMetric(metricRequestTotal).Inc([]string{strconv.Itoa(w.Status())})
	...
	// set request total seconds
	_ = m.GetMetric(metricRequestTotalSeconds).Add([]string{ctx.FullPath(), r.Method, strconv.Itoa(w.Status())}, latency.Seconds())
	...
	if w.Status() >= 400 {
		_ = m.GetMetric(metricErrorRequest).Inc([]string{ctx.FullPath(), r.Method})
	} else {
		_ = m.GetMetric(metricSuccessRequest).Inc([]string{ctx.FullPath(), r.Method})
	}
